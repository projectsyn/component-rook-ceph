...
This parameter controls whether the RBD CSI driver and any configured `CephBlockPool` resources and associated storage- and volumesnapshotclasses are provisioned.

The `CephBlockPool` resources are defined and configured in parameter <<_xrf_TODO,`storage_pools.rbd`>>.
...
This parameter controls whether the CephFS CSI driver and any configured `CephFilesystem` resources and associated storage- and volumesnapshotclasses are provisioned.

The `CephFilesystem` resources are defined and configured in parameter <<_xrf_TODO,`storage_pools.cephfs`>>.
...
=== Configure the component for OCP4

The component automatically configures some parameters for OpenShift 4.
However, the following configurations must be provided through the hierarchy.
The hierarchy may set these parameters in the OpenShift 4 distribution class.
  ...
  rook_ceph:
    ...
    cephClusterSpec:
      disruptionManagement:
        manageMachineDisruptionBudgets: true <2>
----
...
<2> Tell the operator to create and manage MachineDisruptionBudgets.
